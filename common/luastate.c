...
static void stack_init(struct lua_State* L) {
    ...
    L->top++;  // the first empty stack to be filled/pushed
...
struct TValue* index2addr(struct lua_State* L, int idx) {
    if (idx >= 0) {
...
lua_Integer lua_tointeger(struct lua_State* L, int idx, int* isnum) {
    ...
    if (addr->tt_ == LUA_NUMINT) {
        ...
        *isnum = 0;
        LUA_ERROR(L, "cannot convert to ingeter!\n");
    }
    return ret;
}
...
lua_Number lua_tonumberx(struct lua_State* L, int idx, int* isnum) {
    ...
    if (addr->tt_ == LUA_NUMFLT) {
        ...
        *isnum = 0;
        LUA_ERROR(L, "cannot convert to number!\n");
    }
    return ret;
}
