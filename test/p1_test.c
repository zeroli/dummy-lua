...
static int test_main01(struct lua_State* L) {
  ...
  printf("%s luaL_tointeger value = %d\n", __FUNCTION__, i);
  return 0;
}
...
void p1_test_result02() {
  struct lua_State* L = luaL_newstate();
  printf("%s before push args test_main02 stacksize = %d\n",
    __FUNCTION__, luaL_stacksize(L));
  luaL_pushcfunction(L, &test_main02);
  luaL_pushinteger(L, 1);

  printf("%s before call test_main02 stacksize = %d\n",
    __FUNCTION__, luaL_stacksize(L));
  luaL_pcall(L, 1, 0);
  printf("%s after call test_main02 stacksize = %d\n",
    __FUNCTION__, luaL_stacksize(L));
  luaL_close(L);
}
...
void test_all()
{
  p1_test_result01();
  p1_test_result02();
  p1_test_result03(); // nwant = 0; and nresult > 1;
  p1_test_result04(); // nwant = 1; and nresult = 0;
  p1_test_result05(); // nwant = 1; and nresult = 1;
  p1_test_result06(); // nwant = 1; and nresult > 1;
  p1_test_result07(); // nwant > 1; and nresult = 0;
  p1_test_result08(); // nwant > 1; and nresult > 0;
  p1_test_result09(); // nwant = -1; and nresult = 0;
  p1_test_result10(); // nwant = -1; and nresult > 0;

  p1_test_nestcall01(); // call count < LUA_MAXCALLS
  p1_test_nestcall02(); // call count >= LUA_MAXCALLS
}
